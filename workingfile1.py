...
# Initialize Dash app with Bootstrap theme and proper configuration
app = dash.Dash(
    __name__, 
...
# Configure the app to handle duplicate callbacks
...
def download_options(ticker_symbol, opt_type='c', max_days=60, lower_moneyness=0.95, upper_moneyness=1.05):
    try:
        # Add retry logic for ticker initialization
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    print(f"Failed to initialize ticker after {max_retries} attempts: {str(e)}")
                    return pd.DataFrame()
        ...
        try:
            ...
            print(f"Failed to get price data for {ticker_symbol}: {str(e)}")
            return pd.DataFrame()
        ...
        # Add retry logic for options chain
                ...
                if days_to_expiry <= max_days:
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt == max_retries - 1:
                                print(f"Failed to get options chain for {expiry_date_str} after {max_retries} attempts: {str(e)}")
                            time.sleep(1)
            except Exception as e:
                print(f"Error processing expiry date {expiry_date_str}: {str(e)}")
                continue
        ...
        print(f"Error in download_options: {str(e)}")
        return pd.DataFrame()
                            ...
                            html.Div(className='table-container', children=[
                                dash_table.DataTable(
                                    ...
                                    style_data_conditional=[
                                        ...
                                        {
                                            'if': {
                                                'filter_query': '{Trade Type} = "Buy"',
                                                'column_id': 'Trade Type'
                                            },
...
# Callbacks
# ---------------------------
# Combined callback for options table
@app.callback(
    [Output('options_table', 'data'),
     Output('options_table', 'selected_rows')],
    [Input('ticker', 'value'),
     Input('opt_type', 'value'),
     Input('max_days', 'value'),
     Input('moneyness', 'value'),
     Input('options_table', 'data_previous'),
     Input('options_table', 'data'),
     Input('options_table', 'selected_rows')],
    prevent_initial_call=False
)
def update_options_table(ticker, opt_type, max_days, moneyness, previous_data, current_data, selected_rows):
    ctx = callback_context
    if not ctx.triggered:
        # Initial load - fetch data for default values
        ticker = 'AAPL'  # Default ticker
        opt_type = 'c'   # Default option type
        max_days = 60    # Default max days
        moneyness = [0.95, 1.05]  # Default moneyness range
    else:
        trigger_id = ctx.triggered[0]['prop_id'].split('.')[0]
        
        # Handle trade type changes
        if trigger_id == 'options_table' and current_data and previous_data:
            changed_rows = []
            for i, (prev, curr) in enumerate(zip(previous_data, current_data)):
                if prev != curr:
                    changed_rows.append(i)
                    
            if changed_rows:
                # Update only the changed rows and preserve trade type
                for i in changed_rows:
                    if 'Trade Type' in current_data[i]:
                        previous_data[i]['Trade Type'] = current_data[i]['Trade Type']
                return previous_data, selected_rows  # Preserve selected rows
    
    # Handle data refresh from inputs
    try:
        if not ticker:
            return [], []
        ...
        # Add BSM and Monte Carlo prices
        df['BSM Price'] = df.apply(lambda row: bsm_price_calc(S, row['strike'], row['Days to Expiry']/365, row['Implied Vol.'], opt_type), axis=1)
        df['Monte Carlo Price'] = df.apply(lambda row: monte_carlo_price_calc(S, row['strike'], row['Days to Expiry']/365, row['Implied Vol.'], opt_type), axis=1)
        
        # Store option type in the data
        df['Option Type'] = opt_type
        
        # Preserve existing trade types and selected rows if possible
        if current_data and len(current_data) == len(df):
            df['Trade Type'] = [row.get('Trade Type', 'Buy') for row in current_data]
            return df.to_dict('records'), selected_rows  # Preserve selected rows
        else:
            df['Trade Type'] = 'Buy'  # Default to Buy for new data
            return df.to_dict('records'), []  # Reset selected rows for new data
            
    except Exception as e:
        print(f"Error in update_options_table: {str(e)}")
        return [], []
...
def update_backtesting_summary(data, selected_rows, iv_adjustment, price_override):
    ...
    for i in selected_rows:
        ...
        opt_type = row.get("Option Type", "c")  # Get option type from the data
...
def update_payoff_diagram(data, selected_rows, opt_type):
    ...
    # Create metrics display with trade type information
    metrics_div = html.Div([
        ...
        html.Div([
            html.Span("Selected Positions: ", style={'fontWeight': 'bold'}),
            html.Span(", ".join([f"{'Buy' if opt['Trade Type'] == 'Buy' else 'Sell'} {opt['Option Type'].upper()} {opt['strike']}" for opt in selected_options]))
        ])
    ])
